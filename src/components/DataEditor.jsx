...
export default function DataEditor({ database, onCreateRow, onDeleteRow, onReload, onUpdateRow }) {
  ...
  async function saveRow(rowIndex) { await onUpdateRow(selected, relation.rows[rowIndex], draftRows[rowIndex], rowIndex); }
  async function removeRow(rowIndex) { await onDeleteRow(selected, relation.rows[rowIndex], rowIndex); }
