...
Students often learn relational algebra as symbols on paper, without seeing how an expression changes a relation at each stage. This project provides a visual, executable learning environment that accepts relational algebra or basic SQL, constructs an Abstract Syntax Tree (AST), validates it, evaluates it in the browser, and displays all intermediate relations. Its browser-first fallback keeps the core visualizer available without a running database server.
...
9. Provide editable relations, a table manager, persistent query history, and persistent theme preference in browser storage when the optional API is unavailable.
10. Optionally connect to a local MySQL API for persistent database edits, schema changes, history, and a SQL workspace.
11. Export the current execution snapshot as PDF, DOCX, and TXT, including source relations, the expression tree, processing steps, intermediate tables, and final output.
12. Provide Learn, Help, Developed By, and Download navigation sections that document the actual interface.

## 5. Non-functional requirements

- Static GitHub Pages deployment; core functionality has no backend or server-side database requirement.
- Responsive interface for desktop and mobile.
- Accessible keyboard-focusable buttons and interactive tree nodes.
- Dark/light theme.
- Client-side execution with no credentials or API keys; optional local MySQL credentials remain in an ignored `.env` file.
- Beginner-maintainable JavaScript module structure.

## 6. Input specifications

Each relation snapshot has an array of `columns` and an array of row objects. Bundled relations include STUDENT, COURSE, FACULTY, ENROLLMENT, STUDENT_COURSES, CSE_STUDENTS, ECE_STUDENTS, and REQUIRED_COURSES; the table manager can add browser-local relations as well.
...
- **Visualization:** D3 AST, selected output relation, raw final-result exports, and PDF/DOCX/TXT execution reports.
- **Explanation:** operation description, condition, tuple metrics, complexity, and operation input.

Below the dashboard are an execution timeline, browser/MySQL-aware data editor, table manager, optional MySQL-only SQL workspace, and persistent query history. The Learn section contains supported-operation explanations, an operation explorer, the required video, and references. Help is a UI-specific user manual; Developed By uses centralized, editable placeholders. Selecting a tree node synchronizes the active timeline step and output table.

## 8. Architecture

```text
GitHub repository → GitHub Pages → static Vite bundle
                                      ├─ React dashboard and Learn / Help / team UI
                                      ├─ D3 tree layout
                                      ├─ Parser → AST → JavaScript execution engine
                                      ├─ SQL-to-RA translator
                                      ├─ PDF / DOCX / TXT and raw result exporters
                                      └─ localStorage fallback (database, history, theme)

Local development may additionally use:

React client → optional Express API → local MySQL database
```
...
- PDF/TXT/DOCX execution reports built from the current snapshot, including the expression tree and actual intermediate results.
- Browser localStorage persistence helpers.

Browser QA additionally verifies the local fallback, a three-step division execution, D3 tree/timeline synchronization, invalid-attribute messaging, report actions, the required embedded video, and persisted theme switching.

Run `npm run test` and `npm run build` before submission.

## 11. Limitations

- SQL-to-relational-algebra translation is intentionally basic; it does not support aggregation, GROUP BY, ORDER BY, subqueries, INSERT, or multiple SQL joins. The separate MySQL SQL workspace is only available with the optional local API.
- Browser fallback table editing keeps relation and primary-key names for the visualizer; it does not model full SQL type constraints or shared multi-user data.
- Browser localStorage is local to one browser/device and can be cleared by the user.
...
- Browser backup/restore and safer schema migration tools.
